...
public class ForeverDistantAction extends AbstractGameAction {
    public static final String ID = masterMod.makeId("ForeverDistantAction");
    public static final UIStrings uiStrings = CardCrawlGame.languagePack.getUIString(ID);
    public static final String[] TEXT = uiStrings.TEXT;
    private final AbstractPlayer p;
    private static final float DURATION = Settings.ACTION_DUR_XFAST;

    public ForeverDistantAction(AbstractCreature target, AbstractCreature source, int amount) {
    ...
    public void update() {
        if (this.duration == DURATION) {
            if (this.p.hand.isEmpty()) {
                ...
                AbstractCard card = new ExcaliburSword();
                if (this.p.hand.getBottomCard().costForTurn > 0) {
                    card.baseDamage += this.amount * this.p.hand.getBottomCard().costForTurn;
                }
                card.applyPowers();
            ...
            if (!AbstractDungeon.handCardSelectScreen.wereCardsRetrieved) {
                ...
                AbstractCard card = new ExcaliburSword();
                if (tmpCard.costForTurn > 0) {
                    card.baseDamage += this.amount * tmpCard.costForTurn;
                }
                card.applyPowers();
