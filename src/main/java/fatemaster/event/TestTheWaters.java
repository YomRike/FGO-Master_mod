...
public class TestTheWaters extends PhasedEvent {
    public static final String ID = masterMod.makeId("TestTheWaters");
    private static final EventStrings eventStrings = CardCrawlGame.languagePack.getEventString(ID);
    private static final String[] DESCRIPTIONS = eventStrings.DESCRIPTIONS;
    private static final String[] OPTIONS = eventStrings.OPTIONS;
    private static final String title = eventStrings.NAME;
    private int goldLoss;
    public TestTheWaters() {
        super(ID, title, "fgoMasterResources/images/event_Master/TestTheWaters.png");
        ...
        if (gold >= this.goldLoss) {
            this.imageEventText.setDialogOption(OPTIONS[0] + this.goldLoss + OPTIONS[3]);
        } else {
            this.imageEventText.setDialogOption(OPTIONS[4] + this.goldLoss + OPTIONS[5], true);
        }
        ...
        switch (this.screenNum) {
            case 0:
                if (buttonPressed == 0) {
                    ...
                    int gold = AbstractDungeon.player.gold;
                    if (gold >= this.goldLoss) {
                        this.imageEventText.updateDialogOption(0, OPTIONS[1] + this.goldLoss + OPTIONS[3]);
                    } else {
                        this.imageEventText.updateDialogOption(0, OPTIONS[4] + this.goldLoss + OPTIONS[5], true);
                    }
